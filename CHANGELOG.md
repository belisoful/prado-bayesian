...
### Packaging
- The committed `composer.json` carries no `path` repository to a local PRADO checkout. It declares the framework's public VCS repository and takes `pradosoft/prado` (`^4.4 || dev-master`) as a development dependency, matching the sibling extensions; an application installing this package supplies the framework itself. Once PRADO 4.4 is released, the constraint can drop `dev-master` and move to `require`.
