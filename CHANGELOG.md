...
### Packaging
- `pradosoft/prado` is a runtime requirement at `^4.4@dev`, not a development one, so Composer installs the framework with the package. The constraint resolves through the branch alias PRADO declares (`dev-master` → `4.4.x-dev`), which is why the package sets `minimum-stability: dev` with `prefer-stable: true`. The committed `composer.json` carries no `path` repository to a local checkout; it declares the framework's public VCS repository, which an application must still list itself because Composer reads `repositories` only from the root project. Once PRADO 4.4 reaches Packagist the constraint can drop `@dev`.
